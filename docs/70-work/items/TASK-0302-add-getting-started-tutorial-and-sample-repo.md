---
id: TASK-0302
type: task
status: ready
priority: medium
owner: null
created: 2026-01-31
updated: null
githubSynced: "2026-01-31T04:23:41Z"
tags:
  - docs
  - onboarding
related:
  specs: []
  adrs: []
  files: []
  prs: []
  issues:
    - "https://github.com/bravellian/workbench/issues/77"
  branches: []
title: add getting started tutorial and sample repo
---

# TASK-0302 - add getting started tutorial and sample repo

## Summary

Imported from GitHub issue: https://github.com/bravellian/workbench/issues/77

## Summary
Imported from GitHub issue: https://github.com/bravellian/workbench/issues/77

## Acceptance criteria
- A “Getting started” doc walks through install, init/scaffold, create a work
  item, and validate the repo.
- A sample repo (or scripted walkthrough in `examples/`) demonstrates the
  workflow with real work items and docs.
- README references the tutorial and sample so new users can find it quickly.

## Acceptance criteria
-
